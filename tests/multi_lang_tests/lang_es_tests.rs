...
mod tests {
    ...
    fn es_durations() {
        fn assert_duration(input: &str, expected_millis: i64) {
            let dur = Dt::from_str_duration(input.trim(), Lang::Es)
    ...
    #[test]
    fn es_output_formatting() {
        let dt: Dt = "2025-01-01".parse().unwrap();

        let out = dt.to_str_b("%a, %d %b %Y", Lang::Es).unwrap();
        assert_eq!(out.as_str(), "Mié, 01 ene 2025");

        let out = dt.to_str_b("%A, %d %B %Y", Lang::Es).unwrap();
        assert_eq!(out.as_str(), "Miércoles, 01 enero 2025");

        let out = dt.to_str_b("%A, %d %B %Y %H:%M:%S", Lang::Es).unwrap();
