use crate::{
    DtErr, DtErrKind, SEC_PER_DAY, SEC_PER_MONTH, SEC_PER_WEEK, SEC_PER_YEAR, TSpan, an_err,
};
...
impl TSpan {
    pub fn from_iso(s: &str) -> Result<TSpan, DtErr> {
        ...
        // Convert accumulated nanoseconds to attoseconds and build TSpan
    ...
    /// constants (the only sensible semantics for a pure `TSpan`).
