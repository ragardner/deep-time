...
impl TSpan {
    /// Converts this `TSpan` to a [`hifitime::Duration`] (nanosecond precision).
    ///
    ...
    /// Converts this `TSpan` to a [`hifitime::Epoch`] using the given `TimeScale`.
    ///
    /// The `TSpan` is interpreted as the **offset from the reference epoch**
