...
impl TSpan {
    /// Creates a `TSpan` from a `hifitime::Duration` (nanosecond precision).
    ///
    /// This is the **exact reverse** of [`TSpan::to_hifitime_duration`].
    #[inline]
    pub fn from_hifitime_duration(dur: Duration) -> Self {
        Self::from_ns(dur.total_nanoseconds())
    }

    /// Creates a `TSpan` from a `hifitime::Epoch` by taking its **raw internal `duration`**.
    ///
