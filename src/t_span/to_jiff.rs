...
impl TSpan {
    /// Converts this `TSpan` to a [`jiff::Span`] (seconds + nanoseconds only).
    ///
    ...
    /// Converts this `TSpan` to a `jiff::SignedDuration` (nanosecond precision).
    ///
    /// - Sub-nanosecond attoseconds are **truncated toward zero**.
    /// - Supports the **entire** range of `TSpan` (never saturates).
    #[inline]
    ...
    /// Converts this `TSpan` to a [`jiff::Timestamp`].
    ///
    /// - Sub-nanosecond attoseconds are **truncated toward zero**.
    /// - The conversion assumes `to_attos()` returns attoseconds since the Unix epoch
    ///   (UTC). Leap-second handling is already performed by `Dt` arithmetic.
