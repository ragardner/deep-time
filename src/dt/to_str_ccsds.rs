...
impl Dt {
    ...
    #[inline]
    pub fn to_str_ccsds(&self, current: Scale, new: Scale) -> Result<String, DtErr> {
        self.to_str_ccsds_nf(current, new, 18)
    }

    /// Same as [`to_str_ccsds`] but lets you control the maximum number of fractional digits (0–18).
    pub fn to_str_ccsds_nf(
        &self,
        current: Scale,
        new: Scale,
        max_precision: usize,
    ...
    #[inline]
    pub fn to_ccsds_doy_str(&self, current: Scale, new: Scale) -> Result<String, DtErr> {
        self.to_ccsds_doy_str_nf(current, new, 18)
    }
