...
#[inline(always)]
pub(crate) fn parse_week_date_no_weekday(normalized: &str, lang: Lang) -> Option<TimePoint> {
    ...
    let classification = classify_date(&normalized.to_ascii_lowercase(), lang).ok()?;
