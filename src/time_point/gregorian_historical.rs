use crate::{
    ClockType, Real, TimePoint, TimeSpan, historical_sofa_offsets::historical_tai_minus_utc_offset,
};

impl TimePoint {
    // =====================================================================
    // SOFA Historical Functions
    // =====================================================================
    ...
    ///   will be accurate.
    /// - **Model consistency warning**: If you later convert the result back
    ///   to UTC using the normal [`Self::to_clock_type`] or
    ///   [`Self::to_gregorian_time`], you may not recover the exact original
    ///   civil time. Use [`Self::to_historical_utc`] instead.
    /// - Calling [`Self::to_historical_utc`] on a `TimePoint` that was **not**
    ///   created with this function (or the normal constructor) may produce
    ///   an incorrect civil time.
    ...
    /// - **Model consistency warning**: Using the normal
    ///   `to_clock_type(ClockType::UTC)` on a pre-1972 instant will generally
    ///   produce a slightly different civil time.
    /// - Calling this function on a `TimePoint` that was **not** created with
    ///   `from_historical_gregorian_ymdhms` may produce an incorrect civil
    ///   time.
    ...
    // =====================================================================
    // SPICE Historical Functions
    // =====================================================================

    /// Creates a `TimePoint` representing the physical instant corresponding
    /// to a civil date using the **SPICE historical model** for dates before
    /// 1 January 1972.
    ///
    /// # SPICE Pre-1972 Model
    ///
    /// Older versions of NASA/NAIF SPICE used a fixed offset of **9 seconds**
    /// between TAI and UTC for all dates prior to the introduction of the
    /// modern leap-second system:
    ///
    /// ```text
    /// TAI = UTC + 9.0   (constant for dates before 1972-01-01)
    /// ```
    ///
    /// This differs from the SOFA model, which uses a gradual ramp.
    ///
    /// # When to use this function
    ///
    /// Use this constructor when you need to exactly reproduce the behavior
    /// of older SPICE software or legacy SPICE-based pipelines. It is
    /// particularly useful for validating against old SPICE output or
    /// working with very old mission data.
    ///
    /// For dates on or after 1 January 1972, this function behaves
    /// identically to [`Self::from_gregorian_ymdhms`].
    ///
    ...
    ///
    /// - The returned `TimePoint` represents the **correct physical instant**
    ///   according to the SPICE historical convention.
    /// - **Model consistency warning**: If you later convert the result back
    ///   to UTC using the normal methods, you will get an incorrect civil
    ///   time for pre-1972 dates. Use [`Self::to_spice_utc`] instead.
    /// - Calling [`Self::to_spice_utc`] on a `TimePoint` that was **not**
    ///   created with this function will generally produce an incorrect
    ///   civil time.
    ...
    /// Converts this instant to UTC using the **SPICE historical model**
    /// for dates before 1 January 1972.
    ///
    /// # When to use this function
    ///
    /// Use this method when you have a `TimePoint` that was originally
    /// created with `from_spice_gregorian_ymdhms` and you need to recover
    /// the historically correct civil time according to older SPICE behavior.
    ///
    /// For dates on or after 1972-01-01, this function behaves identically
    /// to `to_clock_type(ClockType::UTC)`.
    ///
    /// # Important notes and warnings
    ///
    /// - This is the recommended way to round-trip a SPICE-historical
    ///   `TimePoint` back to UTC.
    /// - **Model consistency warning**: Using the normal
    ///   `to_clock_type(ClockType::UTC)` on a pre-1972 instant will generally
    ///   produce a different civil time.
    /// - Calling this function on a `TimePoint` that was **not** created with
