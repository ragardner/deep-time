...
// Table of constants for 2/pi (kept exactly as original)
...
/// Return the last three digits of N with y = x - N*pi/2 so |y| < pi/2.
pub(crate) const fn rem_pio2_large(
    x: &[Real; 3],
    len: usize,
    y: &mut [Real],
    e0: i32,
    ...
    let x1p_24 = Real::from_bits(0x3e70000000000000);
    ...
    let mut ih: i32;
    let mut z: Real;
    let mut f: [Real; 20] = [0.; 20];
    let mut fq: [Real; 20] = [0.; 20];
    let mut q: [Real; 20] = [0.; 20];
    ...
    let mut i: usize = 0;
    while i <= m {
        f[i] = if j < 0 { 0. } else { IPIO2[j as usize] as Real };
        j += 1;
        i += 1;
    }
    ...
    'recompute: loop {
        ...
        let mut j: usize = jz;
        while j >= 1 {
            fw = (x1p_24 * z) as i32 as Real;
            iq[ii as usize] = (z - x1p24 * fw) as i32;
            z = q[j - 1] + fw;
            ii += 1;
            j -= 1;
        }
        ...
        // --- Recompute decision (fixed to match original exactly) ---
        if z == 0.0 {
            let mut bit_or: i32 = 0;
            let mut idx = jk;
            while idx < jz {
                bit_or |= iq[idx];
                idx += 1;
            ...
            if bit_or == 0 {
                // Need more terms from IPIO2.
                // k = number of additional terms required (guarded for safety in const fn)
                let mut k = 1usize;
    ...
    match prec {
        ...
        3 => {
            // Two compensated summation passes (high to low) + tail sum
            let mut i = jz;
            while i >= 1 {
                fw = fq[i - 1] + fq[i];
                fq[i] += fq[i - 1] - fw;
