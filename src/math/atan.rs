...
pub const fn atan(x: Real) -> Real {
    ...
    if ix >= 0x4410_0000 {
        ...
        let z = ATANHI[3] + Real::from_bits(0x0380_0000); // tiny value
        return if sign != 0 { -z } else { z };
    }

    /* id:
     * -1  => |x| < 0.4375  (use polynomial directly)
     *  0  => 7/16  <= |x| < 11/16
     *  1  => 11/16 <= |x| < 19/16
     *  2  => 19/16 <= |x| < 39/16
     *  3  => 39/16 <= |x|
    ...
    let id = if ix < 0x3fdc_0000 {
        ...
        if ix < 0x3e40_0000 {
            /* |x| < 2^-27 */
            if ix < 0x0010_0000 { /* subnormal — original raised underflow via force_eval */ }
            return x;
        }
        -1
    } else {
    ...
    /* break sum from i=0 to 10 of AT[i]*z**(i+1) into odd/even parts */
